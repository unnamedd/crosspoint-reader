...
//!
//! Each symbol is implemented in `src/rust_ffi/`. Keep the two in step: a
//! mismatch here is a link error at best and a corrupt call frame at worst.
//!
//! Nothing outside `backend` should call these directly — the sibling modules
//! wrap them in safe APIs.
//!
//! `safe fn` marks a call that needs no contract from us: it passes no pointer,
//! the C++ side guards its own state, and no argument can reach undefined
//! behaviour. Everything taking a raw pointer stays `unsafe`, so the keyword is
//! left meaning "there is a rule here you must keep" rather than "this crosses
//! into C++", which was true of every line and therefore told you nothing.

use core::ffi::{c_char, c_void};

unsafe extern "C" {
    // -- renderer -----------------------------------------------------------
    pub safe fn cpp_screen_width() -> i32;
    pub safe fn cpp_screen_height() -> i32;
    pub safe fn cpp_clear_screen();
    pub fn cpp_draw_text(x: i32, y: i32, text: *const c_char, font_id: i32, style: u8);
    pub safe fn cpp_draw_rect(x: i32, y: i32, width: i32, height: i32, filled: u8, black: u8);
    pub safe fn cpp_draw_line(x1: i32, y1: i32, x2: i32, y2: i32);
    ...
    /// filled portion.
    pub safe fn cpp_fill_rect_dither(x: i32, y: i32, width: i32, height: i32, light: u8);
    ...
    pub safe fn cpp_scrim(x: i32, y: i32, width: i32, height: i32);
    /// Confines drawing to a rect. A zero width or height lifts the clip.
    pub safe fn cpp_set_clip(x: i32, y: i32, width: i32, height: i32);
    ...
    pub safe fn cpp_draw_icon(role: u8, filled: u8, size: i32, x: i32, y: i32);
    /// Edge length the host would actually draw for this role, or 0 when it
    /// ships nothing for it.
    pub safe fn cpp_icon_size(role: u8, filled: u8, size: i32) -> i32;
    ...
    pub safe fn cpp_resolve_font(family: u8, size_pt: u8) -> i32;
    /// The reader font the user selected, including SD-card fonts.
    pub safe fn cpp_reader_font() -> i32;
    pub fn cpp_text_width(font_id: i32, text: *const c_char, style: u8) -> i32;
    pub safe fn cpp_line_height(font_id: i32) -> i32;

    // -- input --------------------------------------------------------------
    pub safe fn cpp_input_was_pressed(button: u8) -> u8;
    pub safe fn cpp_input_is_pressed(button: u8) -> u8;
    pub safe fn cpp_input_was_released(button: u8) -> u8;
    pub safe fn cpp_input_has_touch() -> u8;
    ...
    pub safe fn cpp_input_touch_released() -> u8;
    pub safe fn cpp_input_swipe() -> u8;
    pub safe fn cpp_input_was_back_gesture() -> u8;
    pub safe fn cpp_input_was_home_gesture() -> u8;
    ...
    pub safe fn cpp_millis() -> u32;

    // -- theme --------------------------------------------------------------
    pub safe fn cpp_theme_metric(metric: u8) -> i32;
    ...
    pub safe fn cpp_theme_draw_progress_bar(
        x: i32,
        y: i32,
        width: i32,
        height: i32,
    ...
    pub safe fn cpp_theme_draw_slider(
        x: i32,
        y: i32,
        width: i32,
        height: i32,
    ...
    pub safe fn cpp_theme_draw_scroll_indicator(
        x: i32,
        y: i32,
        width: i32,
        height: i32,
    ...
    pub safe fn cpp_device_battery_percent() -> i32;
    ...
    pub safe fn cpp_heap_total() -> i32;
    pub safe fn cpp_heap_free() -> i32;
    pub safe fn cpp_heap_largest_block() -> i32;
    pub safe fn cpp_heap_min_free() -> i32;
    ...
    pub safe fn cpp_activity_finish();
    pub safe fn cpp_activity_request_update();
}
