...
_GENERATED_FILENAMES: Set[str] = {
    "I18nKeys.h",
    "I18nStrings.h",
    "I18nStrings.cpp",
    # Generated Rust keys. Skipped for the same reason as the C++ files: it
    # names every key, so counting it as usage would mark all of them used
    # and defeat --strip-unused.
    "strings.rs",
}
...
def find_used_string_keys(
    ...
    """
    Scan C/C++ and Rust source files under *src_dirs* for STR_* identifiers.
    ...
    for src_dir in src_dirs:
        ...
        if not p.is_dir():
            continue
        for f in p.rglob("*"):
            # .rs counts too: Rust screens reference keys by name, e.g.
            # c"STR_ABOUT". Scanning only C/C++ made a Rust-only key look
            # unused, so --strip-unused deleted it and the key itself rendered
            # on screen instead of its translation.
            if f.suffix not in {".cpp", ".h", ".c", ".rs"}:
                continue
...
def generate_strings_header(
    ...
    lines: List[str] = [
        ...
        "// clang-format off",
        "",
        "// Key names sorted for binary search, and the StrId each maps to.",
        "// Lets callers that cannot see the StrId enum - the Rust UI framework -",
        "// look a string up by its stable key name. The numeric StrId values are",
        "// positional and shift whenever a key is inserted, so they must never be",
        "// hardcoded outside this generated code.",
        "extern const char* const STRING_KEY_NAMES[];",
        "extern const uint16_t STRING_KEY_IDS[];",
        "extern const uint16_t STRING_KEY_COUNT;",
        "",
        "namespace i18n_strings {",
        "",
    ]
...
def generate_strings_cpp(
    ...
    # Key-name lookup table, sorted so I18n::getByKey can binary search it.
    # The value stored is the key's positional StrId, which is why this table
    # has to be generated from the same ordered list as the enum itself.
    lines.append("// Key names, sorted for binary search")
    lines.append("const char* const STRING_KEY_NAMES[] = {")
    for key in sorted(string_keys):
        _append_string_entry(lines, key)
    lines.append("};")
    lines.append("")
    lines.append("// StrId for each entry of STRING_KEY_NAMES, in the same order")
    lines.append("const uint16_t STRING_KEY_IDS[] = {")
    key_to_id = {key: index for index, key in enumerate(string_keys)}
    for key in sorted(string_keys):
        lines.append(f"  {key_to_id[key]},")
    lines.append("};")
    lines.append("")
    lines.append(f"const uint16_t STRING_KEY_COUNT = {len(string_keys)};")
    lines.append("")
...
def generate_rust_keys(
    string_keys: List[str],
    output_path: str,
    verbose: bool = False,
) -> None:
    """Generate the Rust key constants used by `tr!`.

    One `&CStr` per key, named exactly as in the YAML. The name is kept
    verbatim so that a screen writing `tr!(STR_ABOUT)` contains the literal
    key text, which is what lets find_used_string_keys see it and keeps
    --strip-unused honest.
    """
    lines: List[str] = [
        "// THIS FILE IS AUTO-GENERATED BY gen_i18n.py. DO NOT EDIT.",
        "//",
        "// Translation keys from lib/I18n/translations/english.yaml.",
        "// Look strings up with the `tr!` macro rather than using these",
        "// directly:",
        "//",
        "//     Text::new(tr!(STR_ABOUT))",
        "//",
        "// A key that does not exist here is a compile error, and a key used in",
        "// source but missing from the YAML fails this generator.",
        "",
        "use core::ffi::CStr;",
        "",
    ]
    ...
    for key in string_keys:
        lines.append(f'pub const {key}: &CStr = c"{key}";')
...
def main(
    ...
    # The Rust key constants are generated by lib/backend_rs/build.rs, so that
    # cargo works without a PlatformIO run. Pass --rust-out to override.
    default_rust_out = None

    if rust_out is None:
        rust_out = default_rust_out
    ...
    try:
        ...
        if report_missing:
            print("  Keys falling back to English (i.e. awaiting translation):")
            for lang_idx, code in enumerate(languages):
                if lang_idx == 0:
                    continue
                missing = [
                    k
                    for k in string_keys
                    if k in inherited_sets[lang_idx] and k not in unused_set
                ]
                if not missing:
                    print(f"    {code}: complete")
                    continue
                print(f"    {code}: {len(missing)}")
                for key in missing:
                    print(f"        {key}")
            print()
        ...
        if rust_out:
            generate_rust_keys(string_keys, rust_out, verbose)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--report-missing",
        action="store_true",
        help="List, per language, the keys that fall back to English",
    )
    parser.add_argument(
        "--rust-out",
        default=None,
        help="Path for the generated Rust key constants "
        "(default: lib/crosspoint_rs/src/strings.rs)",
    )
    parser.add_argument(
        "--strip-unused",
        action="store_true",
